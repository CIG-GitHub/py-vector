...
class TestAggregateSortOrder:
	def test_aggregate_preserves_first_appearance_order(self):
		...
		categories = PyVector(['B', 'A', 'C', 'B', 'A'], name='category')
		values = PyVector([10, 20, 30, 15, 25], name='value')
		
		table = PyTable([categories, values])
	...
	def test_aggregate_multiple_partition_keys_preserves_order(self):
		...
		col1 = PyVector([2, 1, 2, 1], name='num')
		col2 = PyVector(['X', 'Y', 'Y', 'X'], name='letter')
		values = PyVector([100, 200, 300, 400], name='val')
		
		table = PyTable([col1, col2, values])
	...
	def test_aggregate_with_interleaved_groups(self):
		...
		groups = PyVector(['A', 'B', 'A', 'B', 'A', 'C', 'B'], name='group')
		values = PyVector([1, 2, 3, 4, 5, 6, 7], name='value')
		
		table = PyTable([groups, values])
	...
	def test_aggregate_numeric_groups_preserve_order_not_value_order(self):
		...
		numbers = PyVector([3, 1, 2, 3, 1], name='num')
		values = PyVector([10, 20, 30, 40, 50], name='val')
		
		table = PyTable([numbers, values])
	...
	def test_aggregate_with_none_values_in_groups(self):
		...
		groups = PyVector(['A', None, 'B', None, 'A'], name='group')
		values = PyVector([1, 2, 3, 4, 5], name='value')
